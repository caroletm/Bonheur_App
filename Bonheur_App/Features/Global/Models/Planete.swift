...
class Planete: Hashable, Identifiable  {
    ...
    convenience init(from dto: PlaneteDTO) {
        self.init(
            ID: dto.id ?? UUID(),
            nom: dto.nom,
            description: dto.description,
            image: ImageResource(name: dto.image, bundle: .main),
            onboarding: dto.onboardingDescription,
            iconOnboarding: ImageResource(name: dto.iconOnboarding, bundle: .main),
            backgroundPlanete: ImageResource(name: dto.backgroundPlanete, bundle: .main),
            //variables seulement dans le front
            isVisited: false,
            position: CGPoint(x: 0, y: 0),
            circleSize: CGSize(width: 140, height: 140),
            circleRelativeOffset: CGPoint(x: 0, y: 0),
            route: .planeteUserTest
        )
    }
